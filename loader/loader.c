...
static VkResult ReadManifestsFromD3DAdapters(const struct loader_instance *inst, char **reg_data, PDWORD reg_data_size, const wchar_t* value_name)
{
    ...
    VULKAN_D3DKMT_ENUMADAPTERS2 adapters = {
        .NumAdapters = 0,
        .pAdapters = NULL
    };
    VULKAN_D3DDDI_QUERYREGISTRY_INFO *full_info = NULL;
    size_t full_info_size = 0;
    char *json_path = NULL;
    size_t json_path_size = 0;

    PFN_VulkanD3DKMTEnumAdapters2 VulkanD3DKMTEnumAdapters2 =
        (PFN_VulkanD3DKMTEnumAdapters2) GetProcAddress(GetModuleHandle("gdi32.dll"), "D3DKMTEnumAdapters2");
    PFN_VulkanD3DKMTQueryAdapterInfo VulkanD3DKMTQueryAdapterInfo =
        (PFN_VulkanD3DKMTQueryAdapterInfo) GetProcAddress(GetModuleHandle("gdi32.dll"), "D3DKMTQueryAdapterInfo");
    if (VulkanD3DKMTEnumAdapters2 == NULL || VulkanD3DKMTQueryAdapterInfo == NULL) {
        goto out;
    }

    // Get all of the adapters
    NTSTATUS status = VulkanD3DKMTEnumAdapters2(&adapters);
    if (status == STATUS_SUCCESS && adapters.NumAdapters > 0) {
        adapters.pAdapters = loader_instance_heap_alloc(inst, sizeof(VULKAN_D3DKMT_ADAPTERINFO) * adapters.NumAdapters, VK_SYSTEM_ALLOCATION_SCOPE_COMMAND);
        if (adapters.pAdapters == NULL) {
            goto out;
        }
        status = VulkanD3DKMTEnumAdapters2(&adapters);
    }
    ...
    for (ULONG i = 0; i < adapters.NumAdapters; ++i) {
        ...
        VULKAN_D3DDDI_QUERYREGISTRY_INFO filename_info = {
            .QueryType = VULKAN_D3DDDI_QUERYREGISTRY_ADAPTERKEY,
            .QueryFlags = {
                .TranslatePath = true,
            },
        ...
        VULKAN_D3DKMT_QUERYADAPTERINFO query_info = {
            .hAdapter = adapters.pAdapters[i].hAdapter,
            .Type = VULKAN_KMTQAITYPE_QUERYREGISTRY,
            .pPrivateDriverData = &filename_info,
            .PrivateDriverDataSize = sizeof(filename_info),
        };
        status = VulkanD3DKMTQueryAdapterInfo(&query_info);
        ...
        if (status != STATUS_SUCCESS || filename_info.Status != VULKAN_D3DDDI_QUERYREGISTRY_STATUS_BUFFER_OVERFLOW) {
            continue;
        }

        while (status == STATUS_SUCCESS && ((VULKAN_D3DDDI_QUERYREGISTRY_INFO*)query_info.pPrivateDriverData)->Status == VULKAN_D3DDDI_QUERYREGISTRY_STATUS_BUFFER_OVERFLOW) {
            bool needs_copy = (full_info == NULL);
            size_t full_size = sizeof(VULKAN_D3DDDI_QUERYREGISTRY_INFO) + filename_info.OutputValueSize;
            ...
            if (needs_copy) {
                memcpy(full_info, &filename_info, sizeof(VULKAN_D3DDDI_QUERYREGISTRY_INFO));
            }
        ...
        if (status != STATUS_SUCCESS || full_info->Status != VULKAN_D3DDDI_QUERYREGISTRY_STATUS_SUCCESS) {
            goto out;
        }
